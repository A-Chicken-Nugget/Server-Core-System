...
public class PlayerTabList {
	...
	private static final HashMap<UUID, PlayerTabList> lookUpTable = new HashMap<>();
	 ...
	 */
	public static PlayerTabList getPlayerList(Player player) {
		if (!lookUpTable.containsKey(player.getUniqueId()))
			return new PlayerTabList(player, SIZE_TWO);
		return lookUpTable.get(player.getUniqueId());
	}

	public PlayerTabList(Player player, int size) {
...
class Skin implements ConfigurationSerializable {
	...
	public static void getSkin(String username, SkinCallBack callBack) {
		...
		if (newcall) {
			...
			}.runTaskAsynchronously(PlayerTabList.plugin);
		}
	}
