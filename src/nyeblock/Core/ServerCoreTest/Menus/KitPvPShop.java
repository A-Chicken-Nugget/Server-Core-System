...
public class KitPvPShop extends ShopBase {
	public KitPvPShop(Main mainInstance, Player player) {
		super(mainInstance,player,"kitpvp_shop");
	}
	...
	public void setContents() {
		...
		// Shop menu
		//
		subMenu = new SubMenu("Kit PvP Shop",27,this);
		...
		shopSubMenu.createShopOption(11, Material.BLUE_WOOL, ChatColor.YELLOW.toString() + ChatColor.BOLD + "Rainbow Scoreboard Title", new ArrayList<String>() {{
			...
			add(ChatColor.YELLOW + "change colors randomly.");
				...
				put(ClickType.LEFT,new Runnable() {
					@Override
					public void run() {
						shopSubMenu.useItem("kitpvp_rainbow_scoreboard_winAction");
					}
				});
		}}, new ArrayList<RequirementBase>() {{
			add(new LevelRequirement(3,Realm.KITPVP));
		}}, "kitpvp_rainbow_scoreboard_winAction", 350, false);
		...
		shopSubMenu.createShopTypeOption(13, Material.FIREWORK_ROCKET, ChatColor.YELLOW.toString() + ChatColor.BOLD + "Fireworks", new ArrayList<String>() {{
			...
			add(new ShopMenuTypeOptionItem("Red","red",450,Arrays.asList(new LevelRequirement(3,Realm.KITPVP))));
			add(new ShopMenuTypeOptionItem("Blue","blue",450,Arrays.asList(new LevelRequirement(4,Realm.KITPVP))));
			add(new ShopMenuTypeOptionItem("Green","green",450,Arrays.asList(new LevelRequirement(5,Realm.KITPVP))));
		}}, new HashMap<ClickType,Runnable>() {{
				put(ClickType.LEFT,new Runnable() {
					@Override
					public void run() {
						shopSubMenu.useItem("kitpvp_fireworks_winAction");
					}
				});
		}}, "Color", "kitpvp_fireworks_winAction", false);
		...
		shopSubMenu.createShopOption(15, Material.CLOCK, ChatColor.YELLOW.toString() + ChatColor.BOLD + "Time Speed Up", new ArrayList<String>() {{
			...
			add(ChatColor.YELLOW + "alternate through day/night quickly.");
				...
				put(ClickType.LEFT,new Runnable() {
					@Override
					public void run() {
						shopSubMenu.useItem("kitpvp_time_speed_up_winAction");
					}
				});
		}}, new ArrayList<RequirementBase>() {{
			add(new LevelRequirement(5,Realm.KITPVP));
		}}, "kitpvp_time_speed_up_winAction", 550, false);
		...
		// Special Kits
		//
		ArrayList<KitBase> kits = mainInstance.getKitHandlingInstance().getRealmKits(Realm.KITPVP,false,true);
	...
	public ItemStack give() {
		...
		shopMeta.setDisplayName(ChatColor.YELLOW + "Kit PvP Shop" + ChatColor.GREEN + " (RIGHT-CLICK)");
		shopMeta.setLocalizedName("kitpvp_shop");
